...
bool Analysis::Initialize(const MA5::Configuration& cfg, const std::map<std::string,std::string>& parameters)
{
  ...
  Manager()->AddHisto("Efficiencies_e", 6, 0.0, 7.0);
  plot_Efficiencies_e = new TH1F("Efficiencies_e", "Efficiencies_e", 6, 0.0, 7.0);
  
  Manager()->AddHisto("StMet_e", 100, 0.0, 1000.0);
  plot_StMet_e = new TH1F("StMet_e", "StMet_e", 100, 0.0, 1000.0);
  
  Manager()->AddHisto("PT_Scalar_mus_e", 100, 0.0, 1000.0);
  plot_PT_Scalar_mus_e = new TH1F("PT_Scalar_mus_e", "PT_Scalar_mus_e", 100, 0.0, 1000.0);
  
  Manager()->AddHisto("PT_Vector_mus_e", 100, 0.0, 1000.0);
  plot_PT_Vector_mus_e = new TH1F("PT_Vector_mus_e", "PT_Vector_mus_e", 100, 0.0, 1000.0);
  
  //plots for the case in which W->mu nu_mu
  
  Manager()->AddHisto("Efficiencies_mu", 6, 0.0, 7.0);
  plot_Efficiencies_mu = new TH1F("Efficiencies_mu", "Efficiencies_mu", 6, 0.0, 7.0);
  
  Manager()->AddHisto("StMet_mu", 100, 0.0, 1000.0);
  plot_StMet_mu = new TH1F("StMet_mu", "StMet_mu", 100, 0.0, 1000.0);
  
  Manager()->AddHisto("PT_Scalar_mus_mu", 100, 0.0, 1000.0);
  plot_PT_Scalar_mus_mu = new TH1F("PT_Scalar_mus_mu", "PT_Scalar_mus_mu", 100, 0.0, 1000.0);
  
  Manager()->AddHisto("PT_Vector_mus_mu", 100, 0.0, 1000.0);
  plot_PT_Vector_mus_mu = new TH1F("PT_Vector_mus_mu", "PT_Vector_mus_mu", 100, 0.0, 1000.0);
...
void Analysis::Finalize(const SampleFormat& summary, const std::vector<SampleFormat>& files)
{
  ...
  plot_PT_Scalar_mus_e->SetLineStyle(1);
  plot_PT_Vector_mus_e->SetLineStyle(1);
  
  //plots for the case in which W->mu nu_mu
  plot_Efficiencies_mu->SetLineStyle(1);
  plot_StMet_mu->SetLineStyle(1);
  plot_PT_Scalar_mus_mu->SetLineStyle(1);
  plot_PT_Vector_mus_mu->SetLineStyle(1);
  ...
  plot_PT_Scalar_mus_e->Draw("HIST");
  plot_PT_Vector_mus_e->Draw("HIST");
  ...
  plot_PT_Scalar_mus_mu->Draw("HIST");
  plot_PT_Vector_mus_mu->Draw("HIST"); 
  ...
  plot_StMet_e->Write();
  plot_PT_Scalar_mus_e->Write();
  plot_PT_Vector_mus_e->Write();
  
  //plots for the case in which W->mu nu_mu
  plot_Efficiencies_mu->Write();
  plot_StMet_mu->Write();
  plot_PT_Scalar_mus_mu->Write();
  plot_PT_Vector_mus_mu->Write();
...
bool Analysis::Execute(SampleFormat& sample, const EventFormat& event)
{
	...
	if (FinalStateJs.size() == 2){
		MergedSemiLeptonic(FinalStateJs, FinalStateLeptons, FinalStateNeutrinos, FinalStateBs, dijet1, b_dijet1, event);
	}
	...
	//********************************************************************************************************************************************************************************************
  	if (FinalStateJs.size() == 2){
  		...
  		//check number of final mus (taus) which are necessary for the background
		MAuint32 contador = 0;
		for (MAuint32 i = 0; i < FinalStateLeptons.size(); i++){
			if ((FinalStateLeptons[i]->pdgid()==15) || (FinalStateLeptons[i]->pdgid()==-15)){
				contador = contador + 1;			
			}	
		}
		...
		if (contador >= 2){
	  		...
	  		if ((FinalStateLeptons[index_remaining]->pdgid()==11) || (FinalStateLeptons[index_remaining]->pdgid()==-11)){
				...
				//fills StMet_e, PT_Scalar_mus_e and PT_Vector_mus_e only if the event passed all the cuts.
				if (cuts.back() == true){
					...
					//fills PT_Scalar_mus_e.
					Manager()->FillHisto("PT_Scalar_mus_e", FinalStateLeptons[index_mu1]->momentum().Pt() + FinalStateLeptons[index_mu2]->momentum().Pt());
					plot_PT_Scalar_mus_e->Fill(FinalStateLeptons[index_mu1]->momentum().Pt() + FinalStateLeptons[index_mu2]->momentum().Pt());
					
					//fills PT_Vector_mus_e.
					Manager()->FillHisto("PT_Vector_mus_e", (FinalStateLeptons[index_mu1]->momentum() + FinalStateLeptons[index_mu2]->momentum()).Pt());
					plot_PT_Vector_mus_e->Fill((FinalStateLeptons[index_mu1]->momentum() + FinalStateLeptons[index_mu2]->momentum()).Pt());
				}
			}
			...
			//checks if the graphs of semi-leptonic decay with W->mu nu_mu need to be filled.
	  		if ((FinalStateLeptons[index_remaining]->pdgid()==13) || (FinalStateLeptons[index_remaining]->pdgid()==-13)){
				...
				//fills StMet_mu, PT_Scalar_mus_mu and PT_Vector_mus_mu only if the event passed all the cuts.
				if (cuts.back()==true){
					Double_t total_PT = 0.0;
					for (MAuint32 i = 0 ; i < FinalStateLeptons.size(); i++){
					...
					//fills PT_Scalar_mus_mu.
					Manager()->FillHisto("PT_Scalar_mus_mu", FinalStateLeptons[index_mu1]->momentum().Pt() + FinalStateLeptons[index_mu2]->momentum().Pt());
					plot_PT_Scalar_mus_mu->Fill(FinalStateLeptons[index_mu1]->momentum().Pt() + FinalStateLeptons[index_mu2]->momentum().Pt());
					
					//fills PT_Vector_mus_mu.
					Manager()->FillHisto("PT_Vector_mus_mu", (FinalStateLeptons[index_mu1]->momentum() + FinalStateLeptons[index_mu2]->momentum()).Pt());
					plot_PT_Vector_mus_mu->Fill((FinalStateLeptons[index_mu1]->momentum() + FinalStateLeptons[index_mu2]->momentum()).Pt());
				}
			}
		}	
  	}
