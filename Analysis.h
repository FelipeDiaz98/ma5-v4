 ...
 private:
	...
	//Indicates if the particle given is a final state neutrino.
 	...
 	//Organizes the final state vectors in terms of their PT (greatest to smallest).
      	...
      	Double_t Charge(const MCParticleFormat* particle){
 			...
 			//If the particle is none of these asigns a big value (so it can be known it was none of the ones we want). Just to avoid charge not being defined.
 			else{
 				charge = -5000.0;
 			}
      		}
	...
	void MergedSemiLeptonic(std::vector<const MCParticleFormat*> jets, std::vector<const MCParticleFormat*> leptons, std::vector<const MCParticleFormat*> neutrinos, std::vector<const MCParticleFormat*> bs, MA5::MALorentzVector &dijet1, MA5::MALorentzVector &b_dijet1, const EventFormat& event){
		...
		if (dR_dijet > 0.8){
			...
			//First selection criteria.
			if ((cuts.back() == true) && (lepton.Pt() > 35) && (abs(lepton.Eta()) < 2.4)){
				cuts.push_back(true);
			}
			else{
			...
			std::cout << ((cuts.back() == true) && (lepton.Pt() > 35) && (abs(lepton.Eta()) < 2.4)) << std::endl;
			
			//Second selection criteria.
			if ((cuts.back() == true) && (jet1.Pt() > 30) && (abs(jet1.Eta()) < 5) && (jet2.Pt() > 30) && (abs(jet2.Eta()) < 5)){
				cuts.push_back(true);
			}
			else{
				cuts.push_back(false);
			...
			if ((cuts.back() == true) && (b1.Pt() > 30) && (abs(b1.Eta()) < 2.4) && (b2.Pt() > 30) && (abs(b2.Eta()) < 2.4)){
				cuts.push_back(true);
			}
			else{
				cuts.push_back(false);
			...
			if ((cuts.back() == true) && (PHYSICS->Transverse->EventMET(event.mc()) > 30)){
				cuts.push_back(true);
			}
			else{
				cuts.push_back(false);
			...
			if ((cuts.back() == true) && (mu1.Pt() > 30) && (abs(mu1.Eta()) < 2.4) && (mu2.Pt() > 30) && (abs(mu2.Eta()) < 2.4) && (Charge(leptons[index_mu1]) + Charge(leptons[index_mu2]) < 1.0)){
				cuts.push_back(true);
			}
			else{
				cuts.push_back(false);
